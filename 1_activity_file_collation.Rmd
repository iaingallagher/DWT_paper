...
clin_data <- clin_data %>% select(ag_ID, gender0m1f) %>% rename(subj = ag_ID) #colname in clin data same as colname in activity data
# keep clinical data to just those in activity data, right join to include all rows in clin with match in activity
all_data <- right_join(clin_data, all_data, by = 'subj')
# filter datasets
male_data <- all_data %>% filter(gender0m1f == 0)
female_data <- all_data %>% filter(gender0m1f == 1)
# length(unique(female_data$subj)) # 109 females
# length(unique(male_data$subj)) # 23 males
# write datasets
write_csv(male_data, 'generated_data/male_activity_data.csv')
write_csv(female_data, 'generated_data/female_activity_data.csv')
```

For each of the male and female datasets we will now split the data into files for separate week days.
```{r}
# takes dataset, parses out gender part for file name & splits into weekday specific files
