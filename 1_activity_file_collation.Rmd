...
write_csv(all_data, 'all_collated_activity_data.csv')
```
...
```{r}
# add column to id first & last dates for subj specific filter
...
# test <- one_week_data %>% summarise(n = n_distinct(Date))
# everyone has 7 days
```

Simone Tomasz provided clinical data with details of sex and other clinical characteristics. We will filter the whole dataset down to separate male and female files.
...
# filter datasets
male_data <- one_week_data %>% filter(gender0m1f == 0)
